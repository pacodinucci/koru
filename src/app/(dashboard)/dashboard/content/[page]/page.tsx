...
export default async function DashboardContentPage({ params }: { params: Promise<{ page: string }> }) {
  ...
  const [initialTextMap, cmsPages] = await Promise.all([
    getCmsDraftTextMapBySlug(page.slug),
    discoverPagesGroupRoutes(),
  ]);

  return <DashboardShell userEmail={user.email} cmsPages={cmsPages.filter((item) => !item.isDynamic)} breadcrumbPage={`Contenido / ${page.label}`} contentNoPadding showPanelToggle panelDefaultOpen><AdditionalPageContentEditor pageKey={pageKey as CmsContentPageKey} initialTextMap={initialTextMap} /></DashboardShell>;
}
